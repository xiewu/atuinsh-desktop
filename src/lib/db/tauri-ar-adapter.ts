...
export default function createTauriAdapter<T extends ModelAttributes>(config: TauriAdapterConfig): AdapterConfig<T> {
  ...
  async function all(context: Context, matchOrQuery?: Partial<T> | string, bindValues?: any[]) {
    ...
    if (matchOrQuery === undefined) {
      ...
      whereClause = "WHERE " + Object.keys(matchOrQuery)
        ...
        .join(" AND ");
      bindValues = Object.keys(matchOrQuery).filter((key) => !skippedFields.has(key)).map(key => matchOrQuery[key as keyof T]);
    }
  ...
  async function get(context: Context, primaryKey: any) {
    const res = await context.db.select<T>(`SELECT * FROM ${tableName} WHERE ${getPrimaryKeyField()} = $1`, [primaryKey]);
  ...
  async function insert(context: Context, _model: Model<T>, data: Partial<T>) {
    ...
    const query = `INSERT INTO ${tableName} (${getPrimaryKeyField()}, ${snakeCaseFields.join(", ")}) VALUES (?, ${fields.map(() => "?").join(", ")})`;
  ...
  async function update(context: Context, model: Model<T>, data: Partial<T>) {
    ...
    const fields = Object.keys(data).filter(field => field !== getPrimaryKeyField());
    const query = `UPDATE ${tableName} SET ${fields.map((field) => `${camelCaseToSnakeCase(String(field))} = ?`).join(", ")} WHERE ${getPrimaryKeyField()} = ?`;
  ...
  async function del(context: Context, model: Model<T>) {
    const primaryKey = model.get(getPrimaryKeyField());
    const res = await context.db.execute(`DELETE FROM ${tableName} WHERE ${getPrimaryKeyField()} = ?`, [primaryKey]);
...
export async function setTimestamps<T extends ModelAttributes & WithTimestamps>(_context: Context, model: Model<T>) {
  if (model.persisted) {
